                    ...
                    <tr
                        ...
                        <th scope="row"
                            ...
                            <div class="text-start">
                                ...
                                {{-- CHEETOS CORN STICKS (JAPANESE STEAK FLAVOR) 50 X 60G --}}
                                <div class="font-normal text-gray-500">{{ $item->description }}</div>
                            </div>
                        </th>
                        <td class="px-6">
                            <div id="price{{ $item->carts->first()->id }}" class="text-base font-medium">${{ $item->price }}</div>
                        </td>
                        <td class="px-6">
                            <div class="flex items-center justify-center">
                                {{-- QTY BUTTON --}}
                                <form class="max-w-xs">
                                    {{-- DECREMENT --}}
                                    <div class="relative flex items-center">
                                        <button type="button" onclick="decrementQuantity2('increment-button{{ $item->carts->first()->id }}',
                                        'decrement-button{{ $item->carts->first()->id }}',
                                        'total{{ $item->carts->first()->id }}',
                                        'price{{ $item->carts->first()->id }}')"
                                                id="decrement-button{{ $item->carts->first()->id }}"
                                                class="flex-shrink-0 bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 inline-flex items-center justify-center border border-gray-300 rounded-md h-5 w-5 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                                            <svg class="w-2.5 h-2.5 text-gray-900 dark:text-white" aria-hidden="true"
                                                 xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                                                <path stroke="currentColor" stroke-linecap="round"
                                        ...
                                        <input type="text" id="quantity{{ $item->carts->first()->id }}" data-input-counter
                                               class="flex-shrink-0 text-gray-900 dark:text-white border-0 bg-transparent text-sm font-normal focus:outline-none focus:ring-0 max-w-[2.5rem] text-center"
                                               placeholder="" value="{{ $item->carts->first()->quantity }}" required>

                                        {{-- INCREMENT --}}
                                        <button type="button" onclick="incrementQuantity2('increment-button{{ $item->carts->first()->id }}',
                                        'decrement-button{{ $item->carts->first()->id }}',
                                        'total{{ $item->carts->first()->id }}',
                                        'price{{ $item->carts->first()->id }}')"
                                                id="increment-button{{ $item->carts->first()->id }}"
                                                class="flex-shrink-0 bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 inline-flex items-center justify-center border border-gray-300 rounded-md h-5 w-5 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                                            <svg class="w-2.5 h-2.5 text-gray-900 dark:text-white" aria-hidden="true"
                                                 xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18">
                                                <path stroke="currentColor" stroke-linecap="round"
                        ...
                        <td class="px-6">
                            <div class="text-base font-bold">$<span id="total{{ $item->carts->first()->id }}">65</span></div>
                        </td>
                        <td class="px-6">
                            <div id="" class="text-base font-medium">
                                <button data-modal-target="popup-modal" data-modal-toggle="popup-modal"
